...
// where omega = 2 * pi / wavelen, fi = 2 * pi * speed / wavelen
struct Wave {
    float amp;
    vec2 dir;
    float wavelen;
...
void main()
{
    ...
    for (int i = 0; i < waveCount; ++i) {
        float omega = 2 * pi / waves[i].wavelen;
        float fi = 2 * pi * waves[i].speed / waves[i].wavelen;
    ...
    // for function y = f(x, z), its normal is (-df/dx, 1, -df/dz)
