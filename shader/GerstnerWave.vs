...
void main()
{
    ...
    for (int i = 0; i < waveCount; ++i) {
        float omega = 2 * pi * g / waves[i].l;
        float fi = 2 * pi * waves[i].s / waves[i].l;

        pos.x += waves[i].Q * waves[i].A * waves[i].D.x * 
            cos(dot(waves[i].D, vec2(u,v)) * omega + fi * time);
        pos.y += waves[i].A * sin(dot(waves[i].D, vec2(u,v)) * omega + fi * time);
        pos.x += waves[i].Q * waves[i].A * waves[i].D.y * 
            cos(dot(waves[i].D, vec2(u,v)) * omega + fi * time);
    }
    pos = pos + aPos;
    ...
    // its normal is ( -amp * dir.x * omega * cos(dot(D, xz) * omega + t * fi),
    //                 1 - amp * Q * omega * sin(dot(D, xz) * omega + t * fi),
    //                 -amp * dir.y * omega * cos(dot(D, xz) * omega + t * fi) )
    ...
    for (int i = 0; i < waveCount; ++i) {
        ...
        float fi = 2 * pi * waves[i].s / waves[i].l;
        vec3 n;
        n.x = -waves[i].A * waves[i].D.x * omega
              * cos(dot(waves[i].D, vec2(u,v)) * omega + fi * time);
        n.y =  - waves[i].A * waves[i].Q * omega
              * sin(dot(waves[i].D, vec2(u,v)) * omega + fi * time);
        n.z = -waves[i].A * waves[i].D.y * omega
              * cos(dot(waves[i].D, vec2(u,v)) * omega + fi * time);
        vs_out.normal += n;
    }
